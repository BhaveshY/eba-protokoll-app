...
class SystemAudioRecorder:
    ...
    def has_audio_content(self) -> bool:
        ...
        Samples several 1-second windows spread across the file — the old
        'first 5 seconds only' check falsely flagged meetings as silent when
        system audio did not start at t=0 (a very common case)."""
...
def _assign_speakers_to_segments(diarization, segments: list[dict]) -> list[dict]:
    ...
    Vectorized with NumPy broadcasting — the prior Python double-loop ran
    O(segments * turns) iterations; for a 2h meeting this pairing is the
    dominant CPU cost of post-ASR work."""
    import numpy as np
